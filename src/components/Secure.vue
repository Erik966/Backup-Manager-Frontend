       ...
       <h1 align>File Explorer</h1>
        ...
        >
          ...
          >
            ...
            <v-list-item-content>
              <v-list-item-title @click="on_item_click(item.number)">{{ item.title }}</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-list>
  ...
  mounted(){
   ...
   methods: {
    ...
    for (var i = 0; i < this.currentPath.length; i++)
    {
     ...
     axios.post('http://localhost:5000/fileexplorer', {directory: file_path})
        .then(res => {
          if (res.status === 200) {
            console.log(res.data);
            this.items2 = []
            this.items2.push( {title: "..", directory: true, icon:mdiFolder , number:this.items2.length })
            for (var i = 0; i < res.data.length; i++) {

                let new_val = {title: res.data[i]["name"], directory: res.data[i]["directory"], icon:mdiFileAccount , number:this.items2.length }
